...
export const getTopRated = async (page = 1, perPage = 20) => {
  const {
    data: { results },
  } = await api.get(
...
export const getAiringSchedule = async (perPage) => {
  const { data } = await api.get(
    `/meta/anilist/airing-schedule?perPage=${34}`
  );
...
export const getSearch = async (search, page = 1, perPage = 20) => {
  ...
  const {
    ...
    `/meta/anilist/${search}?page=${page}&perPage=${5}`
  );
...
export const getGenre = async (genres, page = 1, perPage = 24) => {
  if (!genres)
    return {
      error: "No genres",
    };
